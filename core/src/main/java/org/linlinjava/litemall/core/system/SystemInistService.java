...
import org.linlinjava.litemall.db.service.SystemConfigService;
...
class SystemInistService {
    ...
    private void inist() {
        ...
        initConfigs();
        SystemInfoPrinter.printInfo("House_lease 初始化信息", getSystemInfo());
    }
    ...
    static {
        ...
        DEFAULT_CONFIGS.put(SystemConfig.LITEMALL_WX_INDEX_CATLOG_HOUSE, "4");
        DEFAULT_CONFIGS.put(SystemConfig.LITEMALL_WX_SHARE, "false");

        // 合同相关配置默认值

        DEFAULT_CONFIGS.put(SystemConfig.LITEMALL_CONTRACT_UNCONFIRM, "7");
        DEFAULT_CONFIGS.put(SystemConfig.LITEMALL_CONTRACT_COMMENT, "7");

        // 商城相关配置默认值
        DEFAULT_CONFIGS.put(SystemConfig.LITEMALL_MALL_NAME, "house_lease");
    ...
    private Map<String, String> getSystemInfo() {
        ...
        infos.put("首页显示记录数：NEW,HOT,BRAND,TOPIC,CatlogList,CatlogMore",
                SystemConfig.getNewLimit() + "," + SystemConfig.getHotLimit() + "," + SystemConfig.getCatlogListLimit() + "," + SystemConfig.getCatlogMoreLimit());
